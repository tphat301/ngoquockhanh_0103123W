...
$logoFooter = $cache->get("select id, photo, desc$lang options from #_photo where type = ? and act = ? limit 0,1", array('logo-footer', 'photo_static'), 'fetch', 7200);
$banner = $cache->get("select photo from #_photo where type = ? and act = ? limit 0,1", array('banner', 'photo_static'), 'fetch', 7200);
$sloganNew = $cache->get("select name$lang from #_static where type = ? limit 0,1", array('slogan-tin-tuc'), 'fetch', 7200);
$sloganSpnb = $cache->get("select name$lang from #_static where type = ? limit 0,1", array('slogan-spnb'), 'fetch', 7200);
$sloganTc = $cache->get("select name$lang from #_static where type = ? limit 0,1", array('slogan-tieu-chi'), 'fetch', 7200);
...
$prokm = $cache->get("select name$lang, slugvi, slugen, id, photo, regular_price, sale_price, discount, type from #_product where type = ? and find_in_set('khuyenmai',status) and find_in_set('noibat',status) and find_in_set('hienthi',status) order by id desc", array('san-pham'), 'result', 7200);
