...
<?php if ($countItems) { ?>
    <div class="flex-cus row">
        <?php foreach ($items as $k => $v) { ?>
            <div class="col-sm-6 col-md-4 col-lg-3 mg-cus mb-3">
                <div class="box-product box-product-ed" data-aos="fade-up" data-aos-duration="1000">
                    <div class="pic-product pic-product-ed">
                        <a class="text-decoration-none scale-img hover_xam" href="<?= $v[$sluglang] ?>" title="<?= $v['name' . $lang] ?>">
                            <img class="lazy w-100" onerror="this.src='<?= THUMBS ?>/262x252x1/assets/images/noimage.png';" data-src="<?= WATERMARK ?>/product/262x252x1/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" alt="<?= $v['name' . $lang] ?>" title="<?= $v['name' . $lang] ?>" />
                        </a>
                    </div>
